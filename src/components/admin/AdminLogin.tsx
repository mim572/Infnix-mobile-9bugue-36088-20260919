...
export default function AdminLogin({ onLogin }: Props) {
  ...
  const handleAddRedeemCode = async (e: React.FormEvent) => {
    ...
    if (normalizedCode && !/^[A-Z0-9-]{3,20}$/.test(normalizedCode)) {
      toast.error('Invalid code format (letters, numbers, hyphens only, 3-20 chars)');
      return;
    }
    ...
    try {
      const created = await createRedeemCode(amt, uses, normalizedCode || undefined);

      if (created) {
        toast.success(`Redeem code "${created.code}" created successfully!`);
        setRedeemCode('');
        setAmount('500');
        setMaxUses('1');
        setShowRedeemSection(false);
      } else {
        toast.error('Failed to create redeem code. It may already exist or be invalid.');
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Unable to create redeem code');
    } finally {
            ...
            {showRedeemSection && (
              <form onSubmit={handleAddRedeemCode} className="mt-4 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-foreground mb-2">Amount (UGX)</label>
                    <input
                      type="number"
                      value={amount}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-foreground mb-2">Max Uses</label>
                    <input
                      type="number"
                      value={maxUses}
                ...
                <div>
                  <label className="block text-xs font-medium text-foreground mb-2">Custom Code (Optional)</label>
                  <input
                    type="text"
                    value={redeemCode}
                    onChange={e => setRedeemCode(e.target.value)}
                    placeholder="e.g., SUMMER-2024 or leave blank"
                    className="w-full bg-secondary border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
                  />
