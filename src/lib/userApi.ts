...
async function creditReferralCommissionUser(userId: string, amount: number) {
  try {
    ...
    while (currentId && level < 3) {
      ...
      if (commission > 0) {
        await supabase.from('platform_users').update({ wallet_balance: referrer.wallet_balance + commission, referral_earnings: (referrer.referral_earnings || 0) + commission, total_earnings: (referrer.total_earnings || 0) + commission }).eq('id', currentId);
      }
...
export async function buyPackage(
  ...
  paymentNumber: string = 'Wallet',
  paymentNetwork: string = 'Wallet',
  paymentProof: string = 'Auto-activated — paid from wallet balance'
  ...
  if (walletError) {
    // try refund package
    await supabase.from('investment_packages').delete().eq('user_id', user.id).eq('product_name', product.name).eq('status', 'active').order('submitted_at', { ascending: false }).limit(1);
...
// FIX FOR YOUR BUILD ERROR — this was missing
...
export async function submitRecharge(user: PlatformUser, amount: number, payerName: string, payerPhone: string, network: 'MTN' | 'Airtel', proofMessage: string): Promise<{ error: string | null }> {
...
// ─── Withdrawals ──────────────────────────────────────────────────────────────
export async function requestWithdrawal(user: PlatformUser, amount: number): Promise<{ error: string | null }> {
...
// Alias for old name
export const submitWithdrawal = requestWithdrawal;
...
export async function performCheckIn(userId: string): Promise<{ error: string | null; alreadyDone: boolean }> {
  ...
  if (userData) {
    await supabase.from('platform_users').update({ wallet_balance: userData.wallet_balance + 100, total_earnings: userData.total_earnings + 100, daily_earnings: userData.daily_earnings + 100 }).eq('id', userId);
  }
...
export async function doCheckIn(userId: string): Promise<{ success: boolean; error: string | null }> {
  ...
  if (res.alreadyDone) return { success: false, error: 'Already checked in today.' };
...
// ─── Redeem ───────────────────────────────────────────────────────────────────
export async function redeemCode(userId: string, code: string): Promise<{ amount: number; error: string | null }> {
  ...
  if (userData) {
    await supabase.from('platform_users').update({ wallet_balance: userData.wallet_balance + codeData.amount, total_earnings: userData.total_earnings + codeData.amount }).eq('id', userId);
  }
  return { amount: codeData.amount, error: null };
}

// ─── Wallet ───────────────────────────────────────────────────────────────────
export async function updateWallet(userId: string, walletNetwork: string, walletPhone: string, walletName: string): Promise<{ error: string | null }> {
...
export const saveWallet = updateWallet;

// ─── Team ─────────────────────────────────────────────────────────────────────
export async function getTeamTree(userId: string): Promise<{ l1: PlatformUser[]; l2: PlatformUser[]; l3: PlatformUser[] }> {
...
// ─── Missions ─────────────────────────────────────────────────────────────────
export async function claimMission(userId: string, missionId: string, reward: number): Promise<{ error: string | null }> {
  ...
  const { data: userData } = await supabase.from('platform_users').select('wallet_balance, total_earnings').eq('id', userId).single();
  if (userData) {
    await supabase.from('platform_users').update({ wallet_balance: userData.wallet_balance + reward, total_earnings: userData.total_earnings + reward }).eq('id', userId);
  }
  return { error: null };
}
