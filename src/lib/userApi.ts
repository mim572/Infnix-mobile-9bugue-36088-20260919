...
// ─── Internal: credit referral chain ─────────────────────────────────────────
...
export async function buyPackage(
  ...
  // FIX: only insert columns that exist in your table — no user_name, user_phone, last_income_date
  const { error: insertErr } = await supabase.from('investment_packages').insert({
    user_id: user.id,
  ...
  if (insertErr) {
    await supabase.from('platform_users').update({ wallet_balance: user.wallet_balance }).eq('id', user.id);
    return { error: `Purchase failed: ${insertErr.message}` };
  }

  try {
    await creditReferralCommissionUser(user.id, product.amount);
  } catch (e) {
    console.warn('Referral credit failed, but package was bought:', e);
  }
...
export async function getTeamData(userId: string): Promise<{
  ...
  if (l1Ids.length > 0) {
    const { data: l2 } = await supabase.from('platform_users').select('*').in('referred_by', l1Ids);
  ...
  if (l2Ids.length > 0) {
    const { data: l3 } = await supabase.from('platform_users').select('*').in('referred_by', l2Ids);
