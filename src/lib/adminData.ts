...
// PACKAGES - Includes extendPackage for your current error
export async function getPackages(){const{data}=await supabase.from('user_packages').select('*').order('created_at',{ascending:false});return data||[]}
...
export async function extendPackage(id:string, days:number=30){
  try{
    const{data}=await supabase.from('user_packages').select('expires_at,end_date').eq('id',id).single();
    const base = data?.expires_at || data?.end_date || new Date().toISOString();
    const nd = new Date(base); nd.setDate(nd.getDate()+days);
    const {error}=await supabase.from('user_packages').update({expires_at: nd.toISOString(), end_date: nd.toISOString()}).eq('id',id);
...
export async function blockUser(id:string){try{await supabase.from('platform_users').update({is_blocked:true}).eq('id',id)}catch{}}
export async function unblockUser(id:string){try{await supabase.from('platform_users').update({is_blocked:false}).eq('id',id)}catch{}}

// RECHARGES
export async function getRecharges(){const{data}=await supabase.from('recharge_requests').select('*').order('created_at',{ascending:false});return data||[]}
...
export const deleteRecharges=deleteRecharge;

// WITHDRAWALS
export async function getWithdrawals(){const{data}=await supabase.from('withdrawal_requests').select('*').order('created_at',{ascending:false});return data||[]}
...
export const deleteWithdrawals=deleteWithdrawal;

// MISSIONS & BROADCAST
export async function getMissions(){try{const{data}=await supabase.from('missions').select('*');return data||[]}catch{return[]}}
export async function createMission(d:any){const{data,error}=await supabase.from('missions').insert(d).select().single();if(error)throw error;return data}
...
export async function sendBroadcast(_m:string){}
export async function getBroadcasts(){return[]}
