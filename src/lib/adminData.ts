...
// --- Auth - FIXED WITH YOUR PASSWORD ---
const ADMIN_PASSWORD = "Nabakoozamilly@2323";
...
// --- Redeem Codes - WITH CUSTOM CODE SUPPORT ---
export async function getRedeems(): Promise<RedeemCode[]> {
  ...
  if (error) {
    console.error('getRedeems', error);
    return [];
  }
...
export async function createRedeemCode(amount: number, maxUses: number, customCode?: string): Promise<RedeemCode | null> {
  ...
  if (error) {
    console.error('createRedeemCode', error);
    return null;
  }
  return data as RedeemCode;
}
