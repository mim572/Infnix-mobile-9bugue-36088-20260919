...
export async function getAdminStats(): Promise<AdminStats> {
  try {
    ...
    const totalDeposits = deposits?.reduce((s: any, r: any) => s + (r.amount || 0), 0) || 0;
    const totalWithdrawals = withdrawals?.filter((w: any) => w.status === 'approved').reduce((s: any, r: any) => s + (r.amount || 0), 0) || 0;
...
export async function sendBroadcast(_msg: string) {}
...
export async function createRedeemCode(amount: number, maxUses: number, customCode?: string): Promise<RedeemCode | null> {
  ...
  const { data, error } = await supabase.from('redeem_codes').insert({ code, amount, max_uses: maxUses, used_count: 0, is_active: true, expires_at: expiry }).select().single();
