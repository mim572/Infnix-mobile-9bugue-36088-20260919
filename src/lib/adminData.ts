...
// Stats - FIXED to use investment_packages and no head:true
export async function getAdminStats(): Promise<AdminStats> {
 try{
  const {data:users}=await supabase.from('platform_users').select('id');
  const {data:packages}=await supabase.from('investment_packages').select('status,amount');
  ...
  return {
    ...
    totalActivePackages: packages?.filter((p:any)=>p.status==='active'||p.status==='approved').length || packages?.length || 0,
...
// PACKAGES - FIXED TABLE NAME
export async function getPackages(){const{data}=await supabase.from('investment_packages').select('*').order('created_at',{ascending:false});return attachUsers(data||[])}
