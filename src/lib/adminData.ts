...
export async function extendPackage(id:string,days:number=30){try{const{data}=await supabase.from('user_packages').select('expires_at,end_date').eq('id',id).single();const base=data?.expires_at||data?.end_date||new Date().toISOString();const nd=new Date(base);nd.setDate(nd.getDate()+days);await supabase.from('user_packages').update({expires_at:nd.toISOString(),end_date:nd.toISOString()}).eq('id',id)}catch{}}

// USERS - THIS FIXES YOUR CURRENT SCREENSHOT
