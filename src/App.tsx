...
// Auto-trigger daily income processing every hour
function DailyIncomePoller() {
  useEffect(() => {
    const run = () => {
      console.log('[DailyIncome] Polling for due income...');
      supabase.functions.invoke('process-daily-income', {}).then(({ data, error }) => {
        if (error) {
          console.error('[DailyIncome] Error:', error.message);
        } else if (data?.processed > 0 || data?.expired > 0) {
          console.log('[DailyIncome]', data.message);
        }
      });
    };

    // Run immediately on mount, then every 60 minutes
    run();
