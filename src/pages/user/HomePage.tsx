          ...
          <div className="bg-card border border-white/10 rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl">
            ...
            <div className="px-5 py-4 space-y-2 max-h-64 overflow-y-auto">
              <p className="text-muted-foreground text-xs leading-relaxed">
                Infinix Earnings is Uganda's leading mobile investment platform. Invest in Infinix Mobile product packages and earn guaranteed daily income. Withdraw anytime once you've recharged and your package is approved.
              </p>
            ...
            <div className="px-5 pb-5 space-y-2">
              ...
              <a href="https://t.me/+5hk-VcavLmwyMzFk" target="_blank" rel="noopener noreferrer" onClick={handleCloseAnnouncement} className="w-full flex items-center justify-center gap-2 bg-[#229ED9] hover:bg-[#1b87be] text-white font-semibold py-3.5 rounded-2xl text-sm transition-colors">
                Join Official Support
              </a>
            </div>
          </div>
        </div>
      ...
      <div className="px-5 py-5 space-y-5">
        ...
        <div className="bg-card border border-border rounded-2xl p-4">
          ...
          <div className="flex gap-2">
            <input type="text" value={redeemInput} onChange={e => setRedeemInput(e.target.value.toUpperCase())} placeholder="Enter code (e.g. SEP-ABC123)" className="flex-1 bg-secondary border border-border rounded-xl px-3 py-3 text-white text-sm outline-none placeholder:text-muted-foreground" />
            <button onClick={handleRedeem} disabled={redeemLoading} className="px-4 py-3 bg-accent hover:bg-blue-700 text-white font-semibold rounded-xl text-sm transition-colors disabled:opacity-60 w-[110px]">
        ...
        <div className="bg-gradient-to-r from-purple-700/30 to-blue-800/20 border border-purple-500/30 rounded-2xl p-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => { navigator.clipboard.writeText(referralLink); toast.success('Referral link copied!'); }} className="flex items-center justify-center gap-2 bg-white/10 hover:bg-white/15 text-white font-medium py-3 rounded-xl text-xs transition-colors">
              <Copy className="w-4 h-4" /> Copy Link
            </button>
            <button onClick={() => { const msg = `Join Infinix Earnings Uganda & get UGX 7,000 bonus!\n\nInvest in Infinix Mobile packages and earn daily income. Use my referral link:\n${referralLink}`; navigator.share ? navigator.share({ title: 'My Referral Link', text: msg }) : window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`, '_blank'); }} className="flex items-center justify-center gap-2 bg-white/10 hover:bg-white/15 text-white font-medium py-3 rounded-xl text-xs transition-colors">
        ...
        {hasPendingRecharge && (
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-4 flex items-start gap-3">
            ...
            <div className="flex-1"><p className="text-yellow-400 font-bold text-sm">Recharge Under Review</p><p className="text-muted-foreground text-xs mt-0.5 leading-relaxed">Your recharge is being checked by our team. Please wait for approval.</p></div>
          </div>
        )}
        ...
        <div className="bg-card border border-border rounded-2xl p-4">
          ...
          <div className="space-y-2 text-xs text-muted-foreground">
            <p>💰 Min. Deposit: 15,000 UGX | Min. Withdrawal: 7,000 UGX</p>
            <p>📈 Earn daily income automatically once package is approved</p>
            <p>👥 Referral commission applies when invited users invest</p>
            <p>✅ Daily check-in rewards +200 UGX every 24 hours</p>
          </div>
        </div>
      </div>
      <BottomNav />
