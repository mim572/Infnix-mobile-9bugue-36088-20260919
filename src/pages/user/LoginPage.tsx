...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if ((user as any).is_admin) {
      adminLogin('infinix_adm_2026#secure!panel$access');
      navigate('/admin');
      return;
    }
    setUser(user);
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 px-6 py-8">
        ...
        <p className="text-center text-muted-foreground text-sm mt-6">
          New here?{' '}
          <Link to="/register" className="text-accent font-semibold">Create Account</Link>
        </p>

        <div className="mt-8 bg-card border border-border rounded-xl p-4">
          <p className="text-xs text-muted-foreground text-center leading-relaxed">
            🎁 <span className="text-green-400 font-medium">7,000 UGX registration bonus</span> awaits you!
            Invest and earn daily. Refer friends for up to <span className="text-yellow-400 font-medium">30% commission</span>.
          </p>
        </div>
