...
export default function BuyPackagePage() {
  ...
  // Always load fresh balance so admin-credited recharge shows correctly
  useEffect(() => {
    const refresh = async () => {
  ...
  if (!product || !user) { navigate('/packages'); return null; }
      ...
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6 text-center">
        ...
        <div className="flex gap-3 w-full max-w-sm">
          <button
            ...
            className="flex-1 bg-accent hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl text-sm transition-colors"
          >
            View My Products
          </button>
          <button
            onClick={() => navigate('/home')}
            className="flex-1 bg-secondary text-muted-foreground font-semibold py-3.5 rounded-xl text-sm"
          >
            Go Home
          </button>
        </div>
      ...
      <div className="px-5 py-5 space-y-5">
              ...
              <div className="bg-secondary rounded-xl p-3 text-center">
                ...
                <p className="text-green-400 font-bold text-base">
                  +{product.dailyIncome >= 1000000
                    ? (product.dailyIncome / 1000000).toFixed(1) + 'M'
            ...
            <div className="bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-500/20 rounded-xl px-4 py-3 flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="text-white font-bold text-sm">
                  {product.totalReturn >= 1000000
                    ? `UGX ${(product.totalReturn / 1000000).toFixed(1)}M`
                    : `UGX ${(product.totalReturn / 1000).toFixed(0)}K`}
                </p>
        ...
        <div className={`rounded-2xl border p-4 ${balanceLoading ? 'bg-secondary border-border' : canAfford ? 'bg-green-500/10 border-green-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="text-white font-semibold">{user.wallet_balance.toLocaleString()} UGX</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Investment Cost</span>
              <span className="text-red-400 font-semibold">- {product.amount.toLocaleString()} UGX</span>
            </div>
            <div className="h-px bg-border" />
            <div className="flex justify-between">
              <span className="text-muted-foreground">Balance After</span>
              <span className={`font-bold ${canAfford ? 'text-white' : 'text-red-400'}`}>
                {canAfford ? balanceAfter.toLocaleString() : `Need ${(product.amount - user.wallet_balance).toLocaleString()} more`} UGX
              </span>
            </div>
          </div>
          {!canAfford && (
            <button
              ...
              className="mt-3 w-full bg-red-500/20 hover:bg-red-500/30 border border-red-500/30 text-red-400 font-semibold py-2.5 rounded-xl text-sm transition-colors"
            >
              Recharge Account First →
            </button>
          )}
        </div>
        ...
        {canAfford && (
          <button
            ...
            className={`w-full flex items-center gap-3 p-4 rounded-2xl border transition-colors ${
              confirmed ? 'bg-accent/10 border-accent/30' : 'bg-card border-border'
            }`}
          >
            <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-colors ${
              confirmed ? 'bg-accent border-accent' : 'border-muted-foreground'
            }`}>
            ...
            <p className="text-sm text-left text-muted-foreground leading-relaxed">
              I confirm investing <span className="text-white font-semibold">{product.amount.toLocaleString()} UGX</span> from my wallet into this package. I understand this is non-refundable once activated.
            </p>
          </button>
        )}
        ...
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-4 py-3 flex gap-2.5">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            Investment is deducted from your wallet and activated instantly. Daily income is credited automatically every 24 hours. Package cannot be cancelled once active.
          </p>
        </div>
        ...
        <button
          ...
          disabled={!canAfford || !confirmed || loading || balanceLoading}
          className="w-full flex items-center justify-center gap-2 bg-accent hover:bg-blue-700 text-white font-bold py-4 rounded-2xl text-base disabled:opacity-40 transition-all active:scale-98"
        >
          {loading ? (
            ...
            <>
              ...
              Invest {product.amount.toLocaleString()} UGX Now
            </>
          )}
        </button>
      </div>
